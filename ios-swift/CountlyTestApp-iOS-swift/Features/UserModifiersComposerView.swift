    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                if modifier.takesValue {
                    LabeledField("Value", text: $value,
                                 keyboard: modifier.isNumeric ? .decimalPad : .default)
                }
            } footer: {
                Text(modifier.isNumeric
            ...
            Section {
                ...
                Text("Modifiers stack up until save is called, so several can be applied to the same property before anything is sent.")
            }
        }
    }
