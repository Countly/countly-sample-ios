    ...
    var body: some View {
        FeatureList {
            Section {
                ...
                FootnoteText("The server clears a predefined field when it receives an empty string, and ignores null.")
            }
            ...
            Section {
                ...
                FootnoteText("Nothing leaves the device until save is called, and saving also flushes any pending events first, so a profile change always reaches the server before the events that follow it.")
            }
        }
    }
}
