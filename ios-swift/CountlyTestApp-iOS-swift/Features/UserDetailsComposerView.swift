    ...
    var body: some View {
        Form {
            Section {
                ...
                LabeledField("Birth year", text: $birthYear, keyboard: .numberPad)
                LabeledField("Picture URL", text: $picture, placeholder: "optional")
            } header: {
                Text("Predefined")
            } footer: {
                Text("An empty field is sent as an empty string, which is how the server is told to clear it. Leave a field as it is to send it unchanged.")
            }
            ...
            Section {
                ...
                Text("Nothing leaves the device until save. Saving also flushes pending events first, so the profile always reaches the server before the events that follow it.")
            }
        }
    }
